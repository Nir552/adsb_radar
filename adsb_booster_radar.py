# ==============================================================================
# 📡 PROJECT ADS-B: BOOSTER EDITION
# ==============================================================================
# Optimization:
# 1. Lower Gain (Prevent Saturation near Airport)
# 2. Larger Buffer (Prevent Sample Loss)
# 3. Dynamic Threshold (Catch weak signals)
# ==============================================================================
...
REF_LAT = 32.00
REF_LON = 34.88
# שינוי קריטי 1: הורדת גיין למניעת רוויה (תשחק עם זה בין 20 ל-49)
SDR_GAIN = 35.0  
...
try:
    ...
    sdr.gain = SDR_GAIN
    # קריאת חימום לניקוי באפרים
    sdr.read_samples(4096)
    print(f"✅ Hardware Connected. Gain set to: {sdr.gain} dB")
...
def decode_velocity(data_bits):
    ...
    if subtype in (1, 2) and v_ew_raw and v_ns_raw:
        ...
        speed = math.sqrt(v_ew**2 + v_ns**2) * 1.852
...
def print_dashboard(db, total_packets):
    buf = "\033[H\033[J"
    # הוספתי מונה חבילות כדי לראות שהמערכת באמת עובדת וקולטת משהו
    buf += f"=== 📡 BOOSTER RADAR | Total Packets: {total_packets} ===\n"
    buf += "-" * 95 + "\n"
    buf += f"{'ICAO':<8} | {'CALLSIGN':<10} | {'ALT (m)':<8} | {'SPD':<5} | {'HDG':<4} | {'PKTS':<5} | {'POSITION'}\n"
    buf += "-" * 95 + "\n"
    ...
    for icao, p in sorted_planes:
        ...
        loc = f"{p['lat']:.4f}, {p['lon']:.4f}" if p['lat'] else "\033[93mSearching...\033[0m"
        hdg_str = f"{int(p['hdg'])}°" if p['hdg'] else "-"
        
        # הוספתי עמודת PKTS (כמות הודעות שהתקבלו מהמטוס הזה)
        buf += f"{icao:<8} | {p['cs']:<10} | {p['alt']:<8} | {p['spd']:<5} | {hdg_str:<4} | {p['pkt_count']:<5} | {loc}\n"
...
total_packets_decoded = 0

print("Listening for weak signals...")

try:
    while True:
        # שינוי קריטי 2: קריאת באפר גדול יותר (חצי מגה דגימות) כדי לא לאבד מידע
        raw = sdr.read_samples(512 * 1024)
        mag = np.abs(raw)
        
        # שינוי קריטי 3: סף נמוך יותר (3.5 במקום 4)
        thresh = np.mean(mag) * 3.5
        ...
        for p in peaks:
            ...
            # --- הודעה תקינה! ---
            total_packets_decoded += 1
            icao = format(bits_to_int(bits[8:32]), '06X')
            tc = bits_to_int(bits[32:37])
            
            if icao not in db:
                db[icao] = {'cs':'?', 'alt':'?', 'spd':'?', 'hdg':None, 'lat':None, 'lon':None, 'last':0, 'pkt_count':0}
        ...
        if time.time() - last_print > 0.5:
            current = time.time()
            # מחיקה רק אם עברה דקה וחצי (לתת למטוס צ'אנס לחזור)
            clean_db = {k: v for k, v in db.items() if current - v['last'] < 90}
            db = clean_db
            print_dashboard(db, total_packets_decoded)
            last_print = time.time()
